...
`/server.js`
[What that file does]
Main Express application entrypoint. Sets up middleware, static uploads route, mounts API routes (`/api/*`), health and root endpoints, connects to MongoDB, seeds work types, and conditionally mounts environment-specific routes.
[What is use of that file in prjoject ]
...
[What is use of that file in prjoject ]
Centralized configuration used across the server (mongo URI, ports, upload dir, feature flags such as `enableWorkTypes`).
[Type of that file e.g helper / main endpoint ....etc]
...
`/config/index.js`
[What that file does]
Exports configuration values (mongo URI, port, uploadDir, feature flags such as `enableWorkTypes`).
[What is use of that file in prjoject ]
...
`/middleware/rateLimiter.js`
[What that file does]
Provides rate limiter middleware variants to throttle requests.
[What is use of that file in prjoject ]
...
`/controllers`
[What that folder does]
Contains route handler logic (business rules) for orders, vendors, presence, and work types.
[What is use of that file in prjoject ]
...
[What is use of that file in prjoject ]
Provides route definitions and middleware per group (auth, vendors, orders, presence, work-types).
[Type of that file e.g helper / main endpoint ....etc]
Routing layer
...
`/services`
[What that folder does]
Contains business services such as `orderService`, `notificationService`, and `socketService` which perform lower-level logic and integrate with external systems.
[What is use of that file in prjoject ]
...
`/utils/otpStore.js`
[What that file does]
In-memory OTP sender and verifier (used in development/testing). Stores OTP codes temporarily and optionally returns codes for testing.
[What is use of that file in prjoject ]
Used by `routes/auth.js` for OTP flows in development/testing.
[Type of that file e.g helper / main endpoint ....etc]
Utility (OTP dev)
...
- `POST /api/auth/send-otp` : Send OTP to a mobile number
...
## Recently Added Files & Notes (2025 updates)

- `controllers/proxyController.js` — backend-to-backend proxy handlers (e.g. `/api/proxy/earnings/*`).
- `routes/proxy.js` — mounts proxy endpoints under `/api/proxy` and validates internal keys.
- `patches/API_details.md` — project-level API reference created from developer notes.

Notes:
- Proxy endpoints are for trusted backends; keep internal keys secret.
- The documentation focuses on production API surface; development-only helpers should not be used against production databases.
[ File name ]
`package.json`
[its path ]
`/package.json`
